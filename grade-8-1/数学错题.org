...
\epart 联结 $CP$ ,在点 $P$ 、$Q$ 运动过程中，$ΔOPQ$ 与 $ΔBPC$ 是否全等？ 如果全等，请求出点 $Q$ 的运动速度；如果不全等，请说明理由。
\vspace*{1in}
...
\begin{tikzpicture}
  ...
  \path[draw,name path=l1] (0.5,0) --+ (0, 2.7);
  \fill[red,name intersections={of=l1 and Fn1,by=A}];

  % \coordinate (A1) at (intersection of Fn1 and l1);
  % \coordinate (B1) at (intersection of f-2 and l-1);

  % \foreach \i/\angle in {A1/90, B1/180} {
  %   \node[dot, label={[dot label]\angle:$\i$}] at (\i) {};
  % }
